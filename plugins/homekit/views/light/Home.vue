<template>
  <div class="home">
    ...
    <div v-if="!hasColor" class="device">
      <template>
        <img v-if="lightType === 'color8'" src="../../assets/light/light-color-on.png" />
    ...
    <div v-if="false" class="swipe-content">
      <van-swipe ref="swipe" :loop="false" :show-indicators="false" :width="swipeItemWidth">
        <van-swipe-item v-for="(item,index) in swipeItemColor" :key="index" @click="changColor(item)">
          <div class="round" :style="{background: item.color}">
            <div class="ring" v-if="item.ring"></div>
          </div>
          <div class="name">{{item.name}}</div>
        </van-swipe-item>
      </van-swipe>
      <van-button v-if="swipeItemColor.length>4" class="arrow arrow-left" icon="arrow-left" @click="prev()"></van-button>
      <van-button v-if="swipeItemColor.length>4" class="arrow arrow-right" icon="arrow" @click="next()"></van-button>
    </div>
  </div>
</template>
...
const onImg = require('../../assets/light/light-on.png')
const offImg = require('../../assets/light/light-off.png')
...
export default {
  ...
  data() {
    return {
      ...
      swipeItemWidth: 375,
      swipeItem: [
        { name: '我的日光', color: '#FFD475' },
        { name: '暖阳', color: '#FFE4A7', ring: true },
        { name: '颜色3', color: '#FFF3D8' }
      ],
      swipeItemColor: [
        { name: '我的日光', color: '#FFFEBD' },
        { name: '暖阳', color: '#FFE3CE', ring: true },
        { name: '颜色3', color: '#FFD6EA' },
        { name: '颜色4', color: '#73CFFF' },
        { name: '颜色5', color: '#FFD475' }
      ]
    }
  },
  computed: {
  ...
  mounted() {
    this.createPicker()
    this.swipeItemWidth = document.body.clientWidth * 0.23 || document.documentElement.clientWidth * 0.23
  }
}
</script>
